...
function SecondaryChart({
  ...
  return (
    <section>
      <div className="chart-secondary-toolbar">
        <Select
          aria-label="副图周期"
          value={bar}
          onChange={setBar}
          options={['1m', '5m', '15m', '1H', '4H', '1D'].map((value) => ({
            value,
            label: value,
          }))}
        />
      </div>
      <MarketChart candles={market.candles} candle={market.candle} />
    </section>
  );
}
  ...
  return (
    <>
      <div className="chart-view-toolbar">
        ...
        <Checkbox
          checked={averages}
          onChange={(event) => setAverages(event.target.checked)}
        >
          MA 20 / 60
        </Checkbox>
      </div>
      {view === 'depth' ? (
        ...
        <div className={view === 'dual' ? 'chart-double' : ''}>
          <section>
            <MarketChart
          ...
          {view === 'dual' && (
            <SecondaryChart instrument={instrument} mode={mode} />
          )}
        </div>
      )}
    </>
